...
export const api = {
  ...
  deleteNovel: (token, novelId) => request(`/api/novels/${novelId}`, token, { method: "DELETE" }),
  toggleLike: (token, novelId) => request(`/api/novels/${novelId}/like`, token, { method: "POST" }),
  comments: (token, novelId) => request(`/api/novels/${novelId}/comments`, token),
  addComment: (token, novelId, text) => request(`/api/novels/${novelId}/comments`, token, { method: "POST", body: JSON.stringify({ text }) }),
